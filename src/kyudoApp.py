...
#
def main():
    global Learning_rate, Augment_level
    #
    ...
    if ('-train' in cmds or '-predict' in cmds) and len(case_names) > 0 :
        ...
        if not predict:  
            ...
            log_write(f"[kyudoApp]:L2_lambda={L2_lambda:.5f}, Early_stop={Early_stop}")
            log_write(f"[kyudoApp]:section-option={section}")
            # 学習実行(train)
            train_Model( model, s_frames, np_train, np_valid, batch_size, n_epoch, \
                        r_factor, pth = model_pth, augment_params=augment_params )
            ...
            # 予測実行(predict)
            y_pred = predict_Kyudo( model, x, s_frames )    # (input_frames,)
            y = df_y.to_numpy(dtype=np.int64)               # (input_frames,1)
            y_act = y.reshape(-1)                           # (input_frames,)
            x_sect = df_x['section'].to_numpy(dtype=np.int64).reshape(-1)
            correct = ((y_act == y_pred) & (y_act != 0)).sum()
            total = (y_act != 0).sum()
            print(f"[kyudoApp] y_act={y_act.shape}, y_pred={y_pred.shape}")
            print(f"[kyudoApp] correct={correct}, total={total}, accuracy={correct / total:.4f}")
            print(f"[kyudoApp] non-zero y_pred={y_pred[ (y_pred != 0) ]}")
            print(f"[kyudoApp] non-zero sect  ={x_sect[ (y_pred != 0) ]}")
            
            if augment_params is not None:
                _,warp,_ = augment_params
                if warp is not None:
                    print(f"[kyudoApp]:valid-label expanded.(warp={warp:.1f})")
                    y2 = np.repeat(y, repeats=2, axis=1)    # (input_frames, 2)
                    y2 = expand_label(y2, int(warp))
                    y_act = y2[:, 1].reshape(-1)  # 拡張されたラベルを取得
                    correct = ((y_act == y_pred) & (y_act != 0)).sum()
                    print(f"[kyudoApp] correct={correct}, accuracy={correct / total:.4f}, ex_total={(y_act != 0).sum()}")
                    print(f"[kyudoApp] match-sect  ={x_sect[ ((y_act == y_pred) & (y_act != 0)) ]}")

...
Diduct_params = {
    ...
    's6_split_tm': (('<',2.5, 5),   '会の保持時間が短い.秒'),       # 会の時間がこの値未満の場合に減点する
    's8_split_tm': (('<',1.5, 5),   '残身の保持時間が短い.秒'),     # 残身の時間がこの値未満の場合に減点する
    's8_sl_angle': (('>',5.0, 5),  '弓手の下がりが大きい.度')      # 弓手の下がりがこの値を超える場合に減点する
}
#
# eof

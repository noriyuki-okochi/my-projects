...
def section_started_L9(section_no, myResult:MyResult):
    ...
    mylog.log(INFO, f"    lenSW={int(lenSW)}({thsd.ratio(lenSW):.3f}), anglSW={int(anglSW)}°, anglSE={int(anglSE_)}°, anglEW={int(anglEW_)}°")
    #
    # 次の節への移行条件を判定
    #
    # セクションごとの開始条件を取得
    ...
    # 5-Hiki-wake  ->  6-Kai        
        ...
        if g.Step_counter > 90 :  # 離れアラート設定（仮）
            ...
            if (normR < thsd(PRM[0]) and normL < thsd(PRM[1])) and (normER < thsd(PRM[2]) and normEL < thsd(PRM[3])) :
                ...
                #elif g.Hybrid_model:
                #    g.Step_counter = int(g.Step_counter/10)*10                 # カウンターリセット
...
def section_completed_L9(section_no, myResult:MyResult):
    ...
    if section_no == 1:  
        ...
        mylog.log(INFO, f">>>   [ (normR <= {int(thsd(PRM[0]))} and normS <= {int(thsd(PRM[1]))} and normE <= {int(thsd(PRM[2]))}) and"\
                    + f"  (anglSW >= {int(PRM[3])} and anglSW <= {int(PRM[4])}) ]")
        
        Stkp.push( [(0,PRM[0]), (1,PRM[1]), (2,PRM[2]), (3,PRM[3]), (4,PRM[4]), (5,PRM[5])] )  
        if (normR <= thsd(PRM[0]) and normS <= thsd(PRM[1]) and normE <= thsd(PRM[2])) and (anglSW >= PRM[3] and anglSW <= PRM[4]):
            # 右手首、右肩、右腰の移動が小さく、右肩から右手首のベクトルの角度が下向き垂直に近い場合（足踏み動作完了）
            g.Step_counter += 1
            if (g.Step_counter%10) == PRM[5]: completed = True
